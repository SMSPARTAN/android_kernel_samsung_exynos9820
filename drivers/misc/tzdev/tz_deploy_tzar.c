...
static __ref int tz_deploy_handler(void *arg) {
  ...
  for (retry_cnt = 0; retry_cnt < MAX_LOADING_ATTEMPTS; retry_cnt++) {
    ...
    memcpy(tzar, use_new_tzar ? tzdev_tzar_begin : tzdev_old_tzar_begin,
           use_new_tzar ? size : old_size);

    size = DIV_ROUND_UP(size, PAGE_SIZE) * PAGE_SIZE;
    old_size = DIV_ROUND_UP(old_size, PAGE_SIZE) * PAGE_SIZE;
    ret =
        tzdev_mem_register(tzar, use_new_tzar ? size : old_size, 0, NULL, NULL);
    if (ret < 0) {
    ...
    command.buf_desc.size =
        use_new_tzar ? (unsigned int)size : (unsigned int)old_size;
    ...
    for (attempt = 0; attempt < MAX_CONNECTION_ATTEMPTS; attempt++) {
      ...
      tzdev_print(0,
                  "Failed to connect to startup loader socket, "
                  "error = %d, retrying...\n",
                  ret);
