...
PHY_MODE_USB_HOST); if (dwc->dis_split_quirk) { reg = dwc3_readl(dwc->regs,
DWC3_GUCTL3); reg |= DWC3_GUCTL3_SPLITDISABLE; dwc3_writel(dwc->regs,
DWC3_GUCTL3, reg);
                        }
                }
                break;
...
void dwc3_core_config(struct dwc3 *dwc) {
  ...
  if (dwc->revision >= DWC3_REVISION_220A)
    reg |= (DWC3_GSBUSCFG0_DESWRREQINFO | DWC3_GSBUSCFG0_DATWRREQINFO |
            DWC3_GSBUSCFG0_DESRDREQINFO | DWC3_GSBUSCFG0_DATRDREQINFO);
  ...
  if (dwc->revision >= DWC3_REVISION_190A &&
    ...
    reg |= (DWC3_GRXTHRCFG_USBRXPKTCNTSEL | DWC3_GRXTHRCFG_USBRXPKTCNT(3) |
  ...
  if (dwc->revision > DWC3_USB31_REVISION_120A) {
    ...
    reg |= (DWC3_PM_ENTRY_TIMER_US(0x9) | DWC3_PM_LC_TIMER_US(0x5) |
            DWC3_EN_PM_TIMER_US);
...
static struct dwc3_event_buffer *dwc3_alloc_one_event_buffer(struct dwc3 *dwc,
  ...
  evt->buf = dma_alloc_coherent(dwc->sysdev, length, &evt->dma, GFP_KERNEL);
  if (!evt->buf)
    return ERR_PTR(-ENOMEM);
...
static int dwc3_alloc_scratch_buffers(struct dwc3 *dwc) {
  ...
  dwc->scratchbuf =
      kmalloc_array(dwc->nr_scratch, DWC3_SCRATCHBUF_SIZE, GFP_KERNEL);
  if (!dwc->scratchbuf)
    return -ENOMEM;
...
static int dwc3_setup_scratch_buffers(struct dwc3 *dwc) {
  ...
  scratch_addr =
      dma_map_single(dwc->sysdev, dwc->scratchbuf,
  ...
  ret = dwc3_send_gadget_generic_command(dwc, DWC3_DGCMD_SET_SCRATCHPAD_ADDR_LO,
                                         param);
  if (ret < 0)
    goto err1;
  ...
  ret = dwc3_send_gadget_generic_command(dwc, DWC3_DGCMD_SET_SCRATCHPAD_ADDR_HI,
                                         param);
  if (ret < 0)
    goto err1;
...
static int dwc3_core_ulpi_init(struct dwc3 *dwc) {
  ...
  if (intf == DWC3_GHWPARAMS3_HSPHY_IFC_ULPI ||
      (intf == DWC3_GHWPARAMS3_HSPHY_IFC_UTMI_ULPI && dwc->hsphy_interface &&
...
int dwc3_phy_setup(struct dwc3 *dwc) {
  ...
  case DWC3_GHWPARAMS3_HSPHY_IFC_UTMI_ULPI:
    if (dwc->hsphy_interface && !strncmp(dwc->hsphy_interface, "utmi", 4)) {
...
int dwc3_core_init(struct dwc3 *dwc) {
  ...
  dev_info(dwc->dev, "%s: max speed:%d, hibernation:%d, nr_scratch:%d\n",
           __func__, dwc->maximum_speed, dwc->has_hibernation, dwc->nr_scratch);
...
static void dwc3_get_properties(struct dwc3 *dwc) {
  ...
  dwc->has_lpm_erratum = device_property_read_bool(dev, "snps,has-lpm-erratum");
  device_property_read_u8(dev, "snps,lpm-nyet-threshold", &lpm_nyet_threshold);
  ...
  dwc->dis_u2_freeclk_exists_quirk =
      device_property_read_bool(dev, "snps,dis-u2-freeclk-exists-quirk");
  dwc->dis_del_phy_power_chg_quirk =
      device_property_read_bool(dev, "snps,dis-del-phy-power-chg-quirk");
  dwc->dis_tx_ipgap_linecheck_quirk =
      device_property_read_bool(dev, "snps,dis-tx-ipgap-linecheck-quirk");
  dwc->parkmode_disable_ss_quirk =
      device_property_read_bool(dev, "snps,parkmode-disable-ss-quirk");
  ...
  dwc->dis_split_quirk = device_property_read_bool(dev, "snps,dis-split-quirk");
...
bool dwc3_has_imod(struct dwc3 *dwc) {
  return ((dwc3_is_usb3(dwc) && dwc->revision >= DWC3_REVISION_300A) ||
          (dwc3_is_usb31(dwc) && dwc->revision >= DWC3_USB31_REVISION_120A));
}
...
static int dwc3_probe(struct platform_device *pdev) {
  ...
  /* pm_runtime_get_sync triggers dwc3_core_init, which causes Kernel panic
          because dwc3_core_init is called in pm_runtime_get_sync before
     dwc3_alloc_event_buffers. Move the location to call pm runtime api  */
...
static int dwc3_resume_common(struct dwc3 *dwc) {
  ...
  /* Applying KC OTG and Role switch driver patch  - e40c2ae,
          it's difficult to apply the patch because dwc3_resume func was changed
     in Kernel4.9 */
  case USB_DR_MODE_OTG:
    break;
  case USB_DR_MODE_HOST:
...
static void dwc3_complete(struct device *dev) {
  ...
  if (dwc->current_dr_role == DWC3_GCTL_PRTCAP_HOST && dwc->dis_split_quirk) {
    reg = dwc3_readl(dwc->regs, DWC3_GUCTL3);
