...

{
  ...
  if (new_value != old_value) {
    ...
    audit_log(current->audit_context, GFP_KERNEL, AUDIT_MAC_STATUS,
              ...
              old_value, from_kuid(&init_user_ns, audit_get_loginuid(current)),
...
static int sel_make_bools(struct selinux_fs_info *fsi) {
  ...
  for (i = 0; i < num; i++) {
    ...
    ret =
        security_genfs_sid(fsi->state, "selinuxfs", page, SECCLASS_FILE, &sid);
    if (ret) {
      pr_warn_ratelimited(
...
static int sel_make_classes(struct selinux_fs_info *fsi) {
  ...
  for (i = 0; i < nclasses; i++) {
    ...
    class_name_dir =
        sel_make_dir(fsi->class_dir, classes[i], &fsi->last_class_ino);
...
static int sel_fill_super(struct super_block *sb, void *data, int silent) {
  ...
  fsi->policycap_dir =
      sel_make_dir(sb->s_root, "policy_capabilities", &fsi->last_ino);
